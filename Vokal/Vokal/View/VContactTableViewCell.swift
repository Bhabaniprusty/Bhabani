...
class VContactTableViewCell: UITableViewCell {
    ...
    var cellModel: VContactCellModel? {
        didSet{
            ...
            recordButtonButton.isEnabled = VAudioService.sharedInstance.isRecordingAllowed && (!VAudioService.sharedInstance.isRecording || VAudioService.sharedInstance.currentAudioId == cellModel?.audioIdentifier)
        ...
        if keyPath == Static.isRecordingAllowed{
            ...
            if (isRecording == true) {
                if (VAudioService.sharedInstance.currentAudioId != cellModel?.audioIdentifier) {
                    ...
                    recordButtonButton.setTitle("Stop", for: .normal)
                    playButton.isEnabled = false
                }
